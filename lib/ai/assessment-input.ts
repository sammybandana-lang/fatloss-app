 ...
 */
function yesterdayInEasternTime(): string {
  const now = new Date();
 ...
 * to their precise Eastern date in JS afterward; PostgREST has no clean
 * way to express `(start_time AT TIME ZONE 'America/New_York')::date =`
 * as a `.filter()` call, and adding a Postgres function for this one
 * query isn't worth the schema-change overhead in this app.
 */
function wideUtcWindowAround(easternDateStr: string): { start: string; end: string } {
...
async function getYesterdaysWorkoutStats(
  ...
  const { start, end } = wideUtcWindowAround(easternDateStr);
